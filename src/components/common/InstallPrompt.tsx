...
const getCountKey = (platform: PromptPlatform) =>
  platform === "android"
    ? STORAGE_KEYS.androidPromptCount
    : STORAGE_KEYS.iosPromptCount;
...
const hasReachedPromptLimit = (platform: PromptPlatform) =>
  getPromptCount(platform) >= MAX_PROMPT_DISPLAYS;
...
export const InstallPrompt = () => {
  ...
  if (platform === "ios") {
    return (
      <div className="fixed inset-0 z-50 overflow-y-auto bg-white/95 dark:bg-slate-950/95">
        <div className="min-h-full p-4 sm:p-6">
          <div className="mx-auto max-w-2xl rounded-2xl border border-gray-200 bg-white shadow-xl dark:border-slate-700 dark:bg-slate-900">
            <div className="border-b border-gray-200 p-5 dark:border-slate-700 sm:p-6">
              <h3 className="text-xl font-semibold text-gray-900 dark:text-white">
                Install MEDDY on iPhone
              </h3>
            ...
            <div className="p-5 sm:p-6 space-y-4">
              {needsSafariNotice && (
                <p className="rounded-lg border border-amber-300 bg-amber-50 px-3 py-2 text-sm text-amber-800 dark:border-amber-500/40 dark:bg-amber-500/10 dark:text-amber-200">
                  Open MEDDY in Safari first, then use the steps below.
                </p>
              )}

              <ol className="list-inside list-decimal space-y-2 text-sm text-gray-700 dark:text-slate-200">
              ...
              <div className="rounded-xl border border-gray-200 bg-gray-50 p-2 dark:border-slate-700 dark:bg-slate-950">
                <img
            ...
            <div className="flex flex-col justify-end gap-2 border-t border-gray-200 p-5 dark:border-slate-700 sm:flex-row sm:p-6">
              <Button
                variant="ghost"
                onClick={() => {
                  setShowPrompt(false);
